...
IF Len(ls_msg) > 0 THEN
	...
	Close(parent)
	IF li_ret = 1 THEN
		Close(w_ribbonbar_frame)
	END IF
ELSE
	Messagebox(gs_msg_title, "Saved the settings successfully." + ls_msg)
	IF Not Isvalid(w_ribbonbar_frame) Then 
		Open(w_ribbonbar_frame)
	else
		IF ls_theme <> is_theme THEN
			if isvalid(w_ribbonbar_frame) then w_ribbonbar_frame.dynamic of_closewindow("CloseAll") // close all opening window
			ApplyTheme (is_theme_path + ls_theme)
			is_theme = ls_theme
		END IF
	end  if 
